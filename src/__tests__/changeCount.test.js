...
describe("check changeCount", () => {
  it("from add - to = refresh to", () => {
    ...
    const action = changeCount(
      expectedState.fromCurrency.value,
      expectedState.fromCurrency.text,
      expectedState.rate
    );
  ...
  it("invert: from add - to = refresh to", () => {
    ...
    const action = changeCount(
      expectedState.inCurrency.value,
      expectedState.inCurrency.text,
      expectedState.rate
    );
  ...
  it("from - to add = refresh from", () => {
    const expectedState = Object.assign({}, state, {
      fromCurrency: {
        text: "BTC",
        value: round(divide(6700, state.rate), 5),
        onFocus: false,
      },
